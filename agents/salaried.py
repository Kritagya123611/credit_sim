...
class SalariedProfessional(BaseAgent):
    def __init__(self, economic_class='Middle', financial_personality='Rational_Investor'):
        ...
        profile_attributes = {
            "archetype_name": archetype_name, "risk_profile": risk_profile_category, "risk_score": risk_score,
            "economic_class": economic_class, "financial_personality": financial_personality,
            "employment_status": "Salaried", "employment_verification": "EPFO_Verified", "income_type": "Salary",
            "avg_monthly_income_range": modified_income_range, "income_pattern": "Fixed_Date",
            ...
            "loan_emi_payment_status": "ALWAYS_ON_TIME", "has_insurance_payments": True,
            "insurance_types": ["Health", "Life"], "utility_payment_status": "ALWAYS_ON_TIME",
            "mobile_plan_type": "Postpaid", "device_consistency_score": round(random.uniform(0.90, 0.98), 2),
            "ip_consistency_score": round(random.uniform(0.85, 0.95), 2), "sim_churn_rate": "Low",
            "primary_digital_channels": ["Netbanking", "Mobile_Banking"], "login_pattern": "Structured_Daytime",
            "ecommerce_activity_level": "Medium", "ecommerce_avg_ticket_size": "Medium",
        }
    ...
    def _handle_recurring_debits(self, date, events, context):
        if date.day == self.salary_day and self.dependents and random.random() < self.p2p_transfer_chance:
            recipient = self.dependents[0]
            allowance = getattr(recipient, 'monthly_allowance', self.salary_amount * 0.2)
            context.get('p2p_transfers', []).append({
                'sender': self, 'recipient': recipient, 'amount': allowance, 
                'desc': 'Family Support', 'channel': 'UPI'
            })
    ...
    def act(self, date: datetime, **context):
        ...
        self._handle_recurring_debits(date, events, context)
