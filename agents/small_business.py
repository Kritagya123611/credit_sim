...
class SmallBusinessOwner(BaseAgent):
    def __init__(self, economic_class='Middle', financial_personality='Rational_Investor'):
        ...
        profile_attributes = {
            ...
            "employment_status": "Self-Employed", "employment_verification": "Udyam_Registered",
            "income_type": "Business_Sales", "avg_monthly_income_range": modified_income_range,
            "income_pattern": "Erratic_High_Volume", "savings_retention_rate": "Low",
            "has_investment_activity": True, "investment_types": ["Business_Reinvestment"],
            "has_loan_emi": True if random.random() < class_config['loan_propensity'] else False,
            "loan_emi_payment_status": "Mostly_On_Time", "has_insurance_payments": True,
            "insurance_types": ["Business_Insurance"], "utility_payment_status": "Mostly_On_Time",
            "mobile_plan_type": "Postpaid", "device_consistency_score": round(random.uniform(0.80, 0.90), 2),
            "ip_consistency_score": round(random.uniform(0.88, 0.95), 2), "sim_churn_rate": "Low",
            "primary_digital_channels": ["UPI_for_Business", "Netbanking"], "login_pattern": "Structured_Daytime",
            "ecommerce_activity_level": "Medium", "ecommerce_avg_ticket_size": "High",
        }
    ...
    def _handle_operational_expenses(self, date, events, context):
        if date.day == 28 and self.employees:
            for i, employee in enumerate(self.employees):
                salary = self.employee_salaries[i]
                context.get('p2p_transfers', []).append({
                    'sender': self, 'recipient': employee, 'amount': salary, 
                    'desc': 'Salary', 'channel': 'P2P'
                })
    ...
    def act(self, date: datetime, **context):
        ...
        self._handle_operational_expenses(date, events, context)
